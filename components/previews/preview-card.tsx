...
export function PreviewCard({ card, isEnglish, logoCode }: PreviewCardProps) {
  ...
  const getCardDetailUrl = (setCode: string, number: string): string => {
    ...
    return `https://mtgzh.com/card/${setCode.toUpperCase()}/${baseNumber}?utm_source=shiqidi`;
  };

  // 获取卡图URL
  const getCardImageUrl = (setCode: string, number: string): string => {
    return `https://mtgzh.com/image/large/${setCode.toUpperCase()}/${setCode.toUpperCase()}_${number}.jpg`;
  };
              ...
              <Image
                ...
                onError={(e) => {
                  ...
                  if (!img.src.includes('scryfall.io')) {
                    // 尝试从API获取scryfallId
                    fetch(`https://mtgzh.com/api/v1/card/${hoveredCard.setCode.toUpperCase()}/${hoveredCard.number}`)
                      .then(res => res.json())
                      .then(data => {
                        if (data?.id) {

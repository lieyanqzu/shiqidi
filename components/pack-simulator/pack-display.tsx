...
export function PackDisplay({ packs, onFlippedCardsChange, autoFlipCommon }: PackDisplayProps) {
  ...
  useEffect(() => {
    if (onFlippedCardsChange) {
      onFlippedCardsChange(getFlippedCards());
    }
  }, [flippedCards]);
              ...
              {!isCollapsed && (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                  {pack.cards?.map((card, cardIndex) => {
                    ...
                    const shouldAutoFlip = autoFlipCommon && (card.rarity === 'common' || card.rarity === 'uncommon');
                    
                    return (
                      <div 
                        key={`${packIndex}-${cardIndex}`}
                        ...
                        >
                          ...
                          <div className={`absolute inset-0 backface-hidden rotate-y-180`}>
                            {isFlipped ? (
                              ...
                              >
                                <div className={card.sheet.includes('foil') ? 'foil-effect' : ''}>
                                  <Image

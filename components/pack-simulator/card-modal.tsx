...
export function CardModal({ card, onClose, imageUrl, allCards, onCardChange }: CardModalProps) {
  ...
  const getPrevCardImageUrl = useCallback((): string => {
    ...
    if (prevIndex >= 0) {
      ...
      if (!prevImageUrl) {
        setPrevImageUrl(`https://mtgzh.com/image/large/${prevCard.setCode.toUpperCase()}/${prevCard.setCode.toUpperCase()}_${prevCard.number}.jpg`);
      }
      return prevImageUrl;
    }
    return '';
  ...
  const getNextCardImageUrl = useCallback((): string => {
    ...
    if (nextIndex < allCards.length) {
      ...
      if (!nextImageUrl) {
        setNextImageUrl(`https://mtgzh.com/image/large/${nextCard.setCode.toUpperCase()}/${nextCard.setCode.toUpperCase()}_${nextCard.number}.jpg`);
      }
      return nextImageUrl;
    }
    return '';
